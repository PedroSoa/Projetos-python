...
class Aluno:
    ...
    def salvar(self):
        ...
        values = (self.nome, self.data_nascimento, self.turma_id, self.cpf_save)
    ...
    def listar():
        ...
        for aluno in result:
            print(f"ID: {aluno[0]}, Nome: {aluno[1]}, Data de Nascimento: {aluno[2]}, Turma ID: {aluno[3]}, CPF: {aluno[4]}")
...
class Professor:
    def __init__(self, nome, cpf, materia):
        ...
        self.mate = materia 
    def salvar(self):
        query = "INSERT INTO Professores (Nome, cpf, materia) VALUES (%s, %s, %s)"
        values = (self.nome, self.cpf_save, self.mate)
        cursor.execute(query, values)
        db.commit()
    ...
    def listar():
        ...
        for professor in result:
            print(f"ID: {professor[0]}, Nome: {professor[1]}, CPF: {professor[2]}, Materia: {professor[3]}")
...
class UnidadeEscolar:
    ...
    def listar():
        ...
        for unidade in result:
            print(f"ID: {unidade[0]}, Nome da Unidade: {unidade[1]}, Endereço: {unidade[2]}, CEP: {unidade[3]}")

    @staticmethod
    def alterar(unidade_id, novo_nome, novo_endereco, novo_cep):
        query = "UPDATE UnidadesEscolares SET NomeUnidade = %s, Endereco = %s, CEP = %s WHERE ID = %s"
        values = (novo_nome, novo_endereco, unidade_id, novo_cep)
        cursor.execute(query, values)
        db.commit()
...
if __name__ == "__main__":
    while True:
        ...
        if opcao == "1":
            ...
            cpf_save = input("CPF: ")
            aluno = Aluno(nome, data_nascimento, turma_id, cpf_save)
            aluno.salvar()
            ...
            materia = input("Digite a materia lecionada: ")
            professor = Professor(nome, cpf, materia)
            professor.salvar()
            ...
            unidade = UnidadeEscolar(nome_unidade, endereco, cep)
            unidade.salvar()
